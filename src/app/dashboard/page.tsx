    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-slate-900 rounded-2xl p-6 mb-8 text-white">
          <div className="flex justify-between mb-4">
            ...
            <div className="text-right">
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-emerald-500/20 text-emerald-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl p-6 border">
            <div className="flex justify-between mb-2">
              <p className="text-sm text-gray-500 uppercase">Current Value</p>
              <Wallet className="w-5 h-5 text-emerald-500" />
            </div>
            <p className="text-2xl font-bold">{formatL(summary.currentValue)}</p>
            <p className="text-sm text-emerald-600 mt-1">+{ret}% all time</p>
          </div>
          <div className="bg-white rounded-xl p-6 border">
            <div className="flex justify-between mb-2">
              <p className="text-sm text-gray-500 uppercase">Total Invested</p>
              <Banknote className="w-5 h-5 text-blue-500" />
            </div>
            <p className="text-2xl font-bold">{formatL(summary.totalInvested)}</p>
            <p className="text-sm text-gray-500 mt-1">Across {summary.fundCount} funds</p>
          </div>
          <div className="bg-white rounded-xl p-6 border">
            <div className="flex justify-between mb-2">
              <p className="text-sm text-gray-500 uppercase">Total Returns</p>
              <TrendingUp className="w-5 h-5 text-purple-500" />
            </div>
            <p className="text-2xl font-bold text-emerald-600">+{ret}%</p>
            <p className="text-sm text-gray-500 mt-1">XIRR: 13.7%</p>
          </div>
          <div className="bg-white rounded-xl p-6 border">
            <div className="flex justify-between mb-2">
              <p className="text-sm text-gray-500 uppercase">Monthly SIP</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 border">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <PieIcon className="w-5 h-5 text-emerald-500" /> Asset Allocation
            </h3>
        ...
        <div className="bg-white rounded-xl p-6 border mb-8">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Activity className="w-5 h-5 text-purple-500" /> Monthly Performance
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="D" stackId="1" stroke="#0088FE" fill="#0088FE" fillOpacity={0.6} />
              <Area type="monotone" dataKey="E" stackId="1" stroke="#00C49F" fill="#00C49F" fillOpacity={0.6} />
              <Area type="monotone" dataKey="H" stackId="1" stroke="#FFBB28" fill="#FFBB28" fillOpacity={0.6} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <div className="p-6 border-b">
            <h3 className="text-lg font-semibold flex items-center gap-2">
              <Layers className="w-5 h-5 text-blue-500" /> Holdings
            </h3>
          </div>
              ...
              <tbody className="divide-y">
                {funds.map((f, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-0.5 rounded-full text-xs bg-blue-100 text-blue-800">{f.category}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs ${f.returns >= 0 ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"}`}>
                        {f.returns >= 0 ? "+" : ""}
                        {f.returns}%
                      </span>
                    </td>
