...
export default function DashboardPage() {
  ...
  async function addFund() {
    ...
    if (!purchaseDate) return setMessage("Please select purchase date.")
    ...
    try {
      ...
      const gain = currentValue - invested
      const tax = calculateTax({ invested, currentValue, category, purchaseDate })
    ...
    try {
      ...
      const response = await fetch("/api/claude", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1200,
          system: "You are FolioIQ, a sharp Indian mutual fund portfolio advisor. Give practical portfolio observations in plain English. Do not claim SEBI registration. Include a disclaimer that this is educational and not financial advice.",
          messages: [{
            role: "user",
            content: `Analyse this Indian mutual fund portfolio. Total invested: ${formatCurrency(totals.invested)}. Current value: ${formatCurrency(totals.currentValue)}. After-tax gain: ${formatCurrency(totals.afterTaxGain)}. Tax if sold today: ${formatCurrency(totals.tax)}. Health score: ${totals.healthScore}/100. Holdings: ${JSON.stringify(portfolioSummary)}. Respond in these sections: Verdict, Exit or Reduce, Hold and Watch, Increase SIP, Portfolio Gaps, Tax Efficiency Tip, This Week Action.`
          }],
        }),
      })
      const data = await response.json()
      ...
      <header className="sticky top-0 z-50 border-b border-[#e0dcd5] bg-white/90 px-8 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <a href="/" className="flex items-center gap-3">
            ...
            <div><div className="font-serif text-2xl leading-none">FolioIQ</div><div className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#8a857e]">India MF Intelligence</div></div>
          </a>
          <div className="flex items-center gap-3"><span className="rounded-full border border-[#b6ddc8] bg-[#e8f5ee] px-4 py-2 text-xs font-semibold text-[#1a7a4a]">Live NAV</span><a href="/" className="rounded-full border border-[#d0ccc5] bg-white px-4 py-2 text-xs font-semibold">Home</a></div>
        </div>
      </header>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-[#e0dcd5] bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              {portfolio.length > 0 && <button onClick={clearPortfolio} className="rounded-full border border-[#f5c0bc] bg-[#fdecea] px-5 py-2 text-xs font-semibold text-[#c0392b]">Clear Portfolio</button>}
            </div>
            ...
            <div className="mt-6 grid gap-4">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <Input label="Purchase date" value={purchaseDate} setValue={setPurchaseDate} type="date" />
              </div>
            ...
            <div className="mt-8">
              {!portfolio.length ? <div className="rounded-2xl border border-dashed border-[#d0ccc5] bg-[#fafaf8] p-10 text-center"><p className="font-serif text-2xl">No portfolio yet</p><p className="mt-2 text-sm text-[#8a857e]">Add your first fund to see live NAV and after-tax return.</p></div> : <div className="overflow-hidden rounded-2xl border border-[#e0dcd5]">
                <div className="grid grid-cols-[1.5fr_0.8fr_0.8fr_0.8fr_0.7fr] bg-[#fafaf8] px-4 py-3 text-xs font-semibold uppercase tracking-widest text-[#8a857e]"><div>Fund</div><div>Invested</div><div>Current</div><div>After Tax</div><div></div></div>
                {portfolio.map((fund) => <div key={fund.schemeCode} className="grid grid-cols-[1.5fr_0.8fr_0.8fr_0.8fr_0.7fr] items-center border-t border-[#e0dcd5] px-4 py-4 text-sm"><div><p className="font-semibold">{fund.schemeName}</p><p className="mt-1 text-xs text-[#8a857e]">{fund.amc} | NAV {fund.currentNav.toFixed(2)}</p></div><div>{formatCurrency(fund.invested)}</div><div>{formatCurrency(fund.currentValue)}</div><div className={fund.afterTaxReturnPct >= 0 ? "font-semibold text-[#1a7a4a]" : "font-semibold text-[#c0392b]"}>{formatPercent(fund.afterTaxReturnPct)}</div><div className="text-right"><button onClick={() => removeFund(fund.schemeCode)} className="rounded-full border border-[#f5c0bc] bg-[#fdecea] px-4 py-2 text-xs font-semibold text-[#c0392b]">Remove</button></div></div>)}
              </div>}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-[#b6ddc8] bg-gradient-to-br from-[#f0faf5] to-[#f7f5f0] p-6 shadow-sm">
            ...
            {aiAnalysis && <div className="mt-6 whitespace-pre-wrap rounded-2xl border border-[#e0dcd5] bg-white p-5 text-sm leading-7 text-[#1a1714]">{aiAnalysis}</div>}
          </div>
        </div>
      </section>
    </main>
...
function Stat({ title, value, note, green = false }: { title: string; value: string; note: string; green?: boolean }) {
  return <div className={`rounded-2xl border ${green ? "border-[#b6ddc8] bg-[#f0faf5]" : "border-[#e0dcd5] bg-white"} p-6 shadow-sm`}><p className={`text-xs font-semibold uppercase tracking-widest ${green ? "text-[#1a7a4a]" : "text-[#8a857e]"}`}>{title}</p><h2 className={`mt-4 font-serif text-4xl ${green ? "text-[#1a7a4a]" : ""}`}>{value}</h2><p className="mt-2 text-xs text-[#8a857e]">{note}</p></div>
}

function Input({ label, value, setValue, placeholder, type = "text" }: { label: string; value: string; setValue: (value: string) => void; placeholder?: string; type?: string }) {
