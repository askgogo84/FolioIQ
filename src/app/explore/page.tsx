    ...
    <main className="min-h-screen bg-[#f5f7fb] text-[#171a20]">
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <nav className="hidden gap-6 text-sm font-bold text-slate-500 md:flex"><Link href="/explore" className="text-emerald-600">Mutual Funds</Link><Link href="/calculators/sip">Calculators</Link><Link href="/upload">Upload</Link><Link href="/dashboard-v2">Dashboard</Link></nav>
      ...
      <section className="mx-auto max-w-6xl px-5 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_.9fr]">
          ...
          <div className="grid gap-4 md:grid-cols-2">{categories.map(([title, text, href]) => <Link href={href} key={title} className="rounded-2xl bg-white p-5 shadow-sm"><div className="font-black">{title}</div><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p><div className="mt-4 text-sm font-black text-blue-600">View all →</div></Link>)}</div>
        </div>
      </section>
